...
if __name__ == '__main__':
    ...
    for xunhuan in range(0, 5):
        ...
        for epoch in range(1, args.epochs + 1):
            ...
            if best_acc < acc:
                best_acc = acc
                torch.save(net.state_dict(), "save_model/resnet56_10/resnet56" + "_" + str(time.strftime("%Y-%m-%d_%H:%M:%S", time.localtime())) + "_" + str(epoch) + ".pth")
        ...
        Pd_data_for_covs.to_csv("log/resnet56_10" + "_" + str(time.strftime("%Y-%m-%d_%H:%M:%S", time.localtime())) + ".csv",header=False, index=True)
        Pd_data_for_train = pd.DataFrame(data_for_train)
        Pd_data_for_train.to_csv("log/resnet56_10" + "_" + str(time.strftime("%Y-%m-%d_%H:%M:%S", time.localtime())) + ".csv", header=False, index=True)
        Pd_data_for_test = pd.DataFrame(data_for_test)
        Pd_data_for_test.to_csv("log/resnet56_10" + "_" + str(time.strftime("%Y-%m-%d_%H:%M:%S", time.localtime())) + ".csv", header=False, index=True)
        correct_lei_pd = pd.DataFrame(correct_lei)
        correct_lei_pd.to_csv("log/resnet56_10" + "_" + str(time.strftime("%Y-%m-%d_%H:%M:%S", time.localtime())) + ".csv", header=False, index=True)
        tongji.append([best_acc, float(best_acc)])
        xunhuan = xunhuan + 1

    tongji_pd = pd.DataFrame(tongji)
    tongji_pd.to_csv("log/resnet56_10" + "_" + str(time.strftime("%Y-%m-%d_%H:%M:%S", time.localtime())) + ".csv", header=False, index=True)
